...
async function startServer() {
  ...
  app.post('/api/students/session', (req, res) => {
    ...
    // Check if student already recorded in this class
  ...
  app.post('/api/quiz-results', (req, res) => {
    ...
    // Auto-record student if not yet registered
    let student = db.students.find(
      s => s.name.toLowerCase() === trimmedName.toLowerCase() && s.class.toLowerCase() === trimmedClass.toLowerCase()
    );
    if (!student) {
  ...
  app.get('/api/teacher/me', authenticateTeacher, (req, res) => {
  ...
  app.get('/api/teacher/quiz-results', authenticateTeacher, (req, res) => {
  ...
  app.delete('/api/teacher/quiz-results/:id', authenticateTeacher, (req, res) => {
    const { id } = req.params;
  ...
  // Teacher: Export CSV
  app.get('/api/teacher/export-csv', authenticateTeacher, (req, res) => {
    ...
    // CSV format with UTF-8 BOM for Indonesian characters and Excel compatibility
    let csvContent = '\uFEFF';
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Server Belajar IT SMP berjalan di http://0.0.0.0:${PORT}`);
  });
}
